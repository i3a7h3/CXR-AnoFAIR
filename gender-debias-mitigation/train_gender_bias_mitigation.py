...
def parse_args(input_args=None):
    parser = argparse.ArgumentParser(description="CXR-AnoFAIR: Mitigating Attribute Bias in Chest Radiograph Anonymization")
    ...
    parser.add_argument(
        ...
        default=None,
        help="Path to pre-trained attribute classifier", 
        type=str,
        required=True, 
    )
...
def generate_dynamic_targets(probs, target_ratio, w_uncertainty=False):
    ...
    Args:
        ...
        target_ratio: Target distribution (percentage of class 1)
...
def gen_dynamic_weights(gender_indicators, targets, preds_gender_ori, probs_gender_ori, factor=0.5):
    """Generate dynamic weights for the loss based on gender predictions"""
    weights = []
    for gender_indicator, target, pred_gender_ori, prob_gender_ori in zip(
            gender_indicators, targets, preds_gender_ori, probs_gender_ori):
        if not gender_indicator:
            weights.append(factor)  # Lower weight for images without detected gender
        else:
            if target == -1:
                weights.append(factor)
            elif target == pred_gender_ori:
                weights.append(1.0)  # Full weight for correctly aligned gender
            else:
                weights.append(factor)  # Lower weight for misaligned gender

    weights = torch.tensor(weights, dtype=probs_gender_ori.dtype, device=probs_gender_ori.device)
    return weights

def plot_in_grid(images, save_to, gender_indicators=None, gender_preds=None, pred_class_probs=None):
    """Plot images in a grid for visualization
    
    Args:
        images: torch.Tensor [N,3,H,W]
        save_to: Path to save the visualization
        gender_indicators: Boolean tensor indicating if gender was detected
        gender_preds: Predicted gender
    ...
    if gender_indicators is not None and gender_preds is not None and pred_class_probs is not None:
        for i in range(batch_size):
            ...
            if gender_indicators[i]:
                gender_text = f"G: {'M' if gender_preds[i]==1 else 'F'} ({pred_class_probs[i]:.2f})"
                plt.text(x, y, gender_text, color='white', 
...
@torch.no_grad()
def evaluate_process(args, tokenizer, text_encoder, unet, vae, gender_classifier, 
    ...
    for prompt_idx, (prompt, noise) in enumerate(zip(validation_prompts, val_noises)):
        ...
        gender_logits = gender_classifier(normalized_images)
        gender_probs = F.softmax(gender_logits, dim=-1)
        
        # Calculate gender bias - difference between male and female predictions
        ...
        if accelerator.is_main_process:
            ...
            # Calculate balance metrics (closer to 0.5 is better)
            gender_balance = abs(gender_preds_all.float().mean().item() - 0.5)
...
def main():
    ...
    folder_name = (f"CXR-AnoFAIR_BS-{args.train_images_per_prompt_GPU*accelerator.num_processes}_"
    ...
    # 1. Gender classifier
    gender_classifier = AttributeClassifier(args.attribute_classifier_path)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, data_idx in enumerate(train_dataloader_idxs[epoch]):
            ...
            if global_step == 0:
                evaluate_process(
                    args, tokenizer, text_encoder, unet, vae, gender_classifier,
                    accelerator, weight_dtype, epoch, global_step
                )
            ...
            with torch.no_grad():
                ...
                # Process generated images for gender classification
                resized_images = F.interpolate(images, size=(224, 224), mode='bilinear')
                normalized_images = transforms.Normalize(
                    mean=[0.485, 0.456, 0.406], 
                    std=[0.229, 0.224, 0.225]
                ...
                ################################################
                # Step 2: generate dynamic targets for gender alignment
                gender_targets_all, gender_uncertainty_all = generate_dynamic_targets(
                    gender_probs_all, target_ratio=args.target_gender_ratio, w_uncertainty=True
                )
                
                # Apply uncertainty threshold
                gender_targets_all[gender_uncertainty_all > args.uncertainty_threshold] = -1
                
                # Get local targets
                gender_targets = gender_targets_all[gender_probs.shape[0] * accelerator.local_process_index:
                                                  gender_probs.shape[0] * (accelerator.local_process_index + 1)]
                gender_uncertainty = gender_uncertainty_all[gender_probs.shape[0] * accelerator.local_process_index:
                                                          gender_probs.shape[0] * (accelerator.local_process_index + 1)]
                
                accelerator.print(f"\tGender targets: {(gender_targets != -1).sum().item()}/{gender_targets.shape[0]}")
                ...
                # Get original gender
                gender_logits_ori = gender_classifier(normalized_images_ori)
            ...
            loss_da_i = torch.ones(gender_targets.shape, dtype=weight_dtype, device=accelerator.device) * (-1)  # Distributional Alignment
            loss_sp_i = torch.ones(gender_targets.shape, dtype=weight_dtype, device=accelerator.device) * (-1)  # Semantic Preservation
            loss_dp_i = torch.ones(gender_targets.shape, dtype=weight_dtype, device=accelerator.device) * (-1)  # Diagnostic Preservation
            loss_i = torch.ones(gender_targets.shape, dtype=weight_dtype, device=accelerator.device) * (-1)     # Total loss
            
            idxs_i = list(range(gender_targets.shape[0]))
            N_backward = math.ceil(gender_targets.shape[0] / args.train_batch_size)
            for j in range(N_backward):
                ...
                gender_targets_ij = gender_targets[idxs_ij]
                ...
                # Get gender predictions for generated images
                gender_logits_ij = gender_classifier(normalized_images_ij)
                ...
                idxs_w_gender_targets = (gender_targets_ij != -1).nonzero().view([-1])
                if len(idxs_w_gender_targets) > 0:
                    loss_da_ij_w_targets = CE_loss(gender_logits_ij[idxs_w_gender_targets], 
                                                  gender_targets_ij[idxs_w_gender_targets])
                    loss_da_ij[idxs_w_gender_targets] = loss_da_ij_w_targets
                ...
                # Calculate dynamic weights based on gender alignment
                gender_attr_indicators = torch.ones(len(idxs_ij), dtype=torch.bool, device=accelerator.device)
                dynamic_weights = gen_dynamic_weights(
                    gender_attr_indicators, gender_targets_ij, 
            ...
            if accelerator.is_main_process:
                ...
                for key in ["gender_bias", "gender_balanced"]:
                    if logs_i[key] == []:
                        logs_i.pop(key)
