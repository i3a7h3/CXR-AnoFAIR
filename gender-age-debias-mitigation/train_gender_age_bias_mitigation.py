...
class GenderAgeClassifier(nn.Module):
    ...
    def forward_age(self, x):
        return self.age_model(x)
    
    def forward(self, x):
        # Return both gender and age logits
        gender_logits = self.forward_gender(x)
        age_logits = self.forward_age(x)
...
def parse_args(input_args=None):
    parser = argparse.ArgumentParser(description="CXR-AnoFAIR: Mitigating Gender and Age Bias in Chest Radiograph Anonymization")

    # Experiment setting
    parser.add_argument(
        '--proj_name', 
        default="CXR-AnoFAIR-GenderAge",
        help="Project name",
        type=str, 
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Target ratio for male gender (0.0 = all female, 0.5 = balanced, 1.0 = all male)",
    )
    parser.add_argument(
        "--target_age_ratio",
        type=float,
    ...
    # Gender and age-specific weights
    parser.add_argument(
        '--gender_weight', 
        help="Weight for gender alignment loss", 
        type=float, 
        default=1.0
    )
    parser.add_argument(
        '--age_weight', 
        help="Weight for age alignment loss", 
        type=float, 
        default=1.0
    )
...
@torch.no_grad()
def generate_dynamic_targets_gender_age(probs_gender, probs_age, target_gender_ratio=0.5, target_age_ratio=0.5, w_uncertainty=False):
    """Generate dynamic targets for the distributional alignment loss
    
    Args:
        probs_gender: torch.Tensor [N,2], probabilities for gender (female, male)
        probs_age: torch.Tensor [N,2], probabilities for age (young, old)
        target_gender_ratio: Target ratio for male gender (0-1)
        target_age_ratio: Target ratio for young age (0-1)
        w_uncertainty: Whether to return uncertainty measures
    
    Returns:
        targets_gender_all: Target gender classes
        uncertainty_gender_all: Uncertainty of gender targets (if w_uncertainty=True)
        targets_age_all: Target age classes
        uncertainty_age_all: Uncertainty of age targets (if w_uncertainty=True)
    """
    import scipy.stats
    
    # Process gender and age separately but with consistent logic
    # First, handle gender
    idxs_gender_2_rank = (probs_gender != -1).all(dim=-1)
    probs_gender_2_rank = probs_gender[idxs_gender_2_rank]

    if idxs_gender_2_rank.sum() > 0:
        rank_gender = torch.argsort(torch.argsort(probs_gender_2_rank[:, 1]))  # Sort by male probability
        targets_gender = (rank_gender >= (rank_gender.shape[0] * (1 - target_gender_ratio))).long()

        targets_gender_all = torch.ones([probs_gender.shape[0]], dtype=torch.long, device=probs_gender.device) * (-1)
        targets_gender_all[idxs_gender_2_rank] = targets_gender
        
        if w_uncertainty:
            uncertainty_gender = torch.ones([probs_gender_2_rank.shape[0]], dtype=probs_gender.dtype, device=probs_gender.device) * (-1)
            
            # Calculate uncertainty for male targets (1)
            if (targets_gender == 1).any():
                uncertainty_gender[targets_gender == 1] = torch.tensor(
                    1 - scipy.stats.binom.cdf(
                        (rank_gender[targets_gender == 1]).cpu().numpy(), 
                        probs_gender_2_rank.shape[0], 
                        1 - target_gender_ratio
                    )
                ).to(probs_gender.dtype).to(probs_gender.device)
            
            # Calculate uncertainty for female targets (0)
            if (targets_gender == 0).any():
                uncertainty_gender[targets_gender == 0] = torch.tensor(
                    scipy.stats.binom.cdf(
                        rank_gender[targets_gender == 0].cpu().numpy(), 
                        probs_gender_2_rank.shape[0], 
                        target_gender_ratio
                    )
                ).to(probs_gender.dtype).to(probs_gender.device)
            ...
            uncertainty_gender_all[idxs_gender_2_rank] = uncertainty_gender
    else:
        targets_gender_all = torch.ones([probs_gender.shape[0]], dtype=torch.long, device=probs_gender.device) * (-1)
        if w_uncertainty:
            uncertainty_gender_all = torch.ones([probs_gender.shape[0]], dtype=probs_gender.dtype, device=probs_gender.device) * (-1)
    
    # Second, handle age with similar approach
    idxs_age_2_rank = (probs_age != -1).all(dim=-1)
    probs_age_2_rank = probs_age[idxs_age_2_rank]

    if idxs_age_2_rank.sum() > 0:
        rank_age = torch.argsort(torch.argsort(probs_age_2_rank[:, 0]))  # Sort by young probability
        targets_age = (rank_age >= (rank_age.shape[0] * (1 - target_age_ratio))).long()

        targets_age_all = torch.ones([probs_age.shape[0]], dtype=torch.long, device=probs_age.device) * (-1)
        targets_age_all[idxs_age_2_rank] = targets_age
        
        if w_uncertainty:
            uncertainty_age = torch.ones([probs_age_2_rank.shape[0]], dtype=probs_age.dtype, device=probs_age.device) * (-1)
            
            # Calculate uncertainty for young targets (0)
            if (targets_age == 0).any():
                uncertainty_age[targets_age == 0] = torch.tensor(
                    1 - scipy.stats.binom.cdf(
                        (rank_age[targets_age == 0]).cpu().numpy(), 
                        probs_age_2_rank.shape[0], 
                        1 - target_age_ratio
                    )
                ).to(probs_age.dtype).to(probs_age.device)
            
            # Calculate uncertainty for old targets (1)
            if (targets_age == 1).any():
                uncertainty_age[targets_age == 1] = torch.tensor(
                    scipy.stats.binom.cdf(
                        rank_age[targets_age == 1].cpu().numpy(), 
                        probs_age_2_rank.shape[0], 
                        target_age_ratio
                    )
                ).to(probs_age.dtype).to(probs_age.device)
            
            uncertainty_age_all = torch.ones([probs_age.shape[0]], dtype=probs_age.dtype, device=probs_age.device) * (-1)
            uncertainty_age_all[idxs_age_2_rank] = uncertainty_age
    else:
        targets_age_all = torch.ones([probs_age.shape[0]], dtype=torch.long, device=probs_age.device) * (-1)
        if w_uncertainty:
            uncertainty_age_all = torch.ones([probs_age.shape[0]], dtype=probs_age.dtype, device=probs_age.device) * (-1)
    ...
    if w_uncertainty:
        return targets_gender_all, uncertainty_gender_all, targets_age_all, uncertainty_age_all
    else:
        return targets_gender_all, targets_age_all

def gen_dynamic_weights(attribute_indicators, targets_gender, preds_gender_ori, probs_gender_ori, 
                        targets_age, preds_age_ori, probs_age_ori, factor=0.5):
    """Generate dynamic weights for the loss based on gender and age predictions"""
    weights = []
    for attr_indicator, target_gender, pred_gender_ori, prob_gender_ori, target_age, pred_age_ori, prob_age_ori in zip(
            attribute_indicators, targets_gender, preds_gender_ori, probs_gender_ori, 
            targets_age, preds_age_ori, probs_age_ori):
        
        if not attr_indicator:
            weights.append(factor)  # Lower weight for images without attributes detected
        else:
            gender_match = (target_gender == -1) or (target_gender == pred_gender_ori)
            age_match = (target_age == -1) or (target_age == pred_age_ori)
            
            if gender_match and age_match:
                weights.append(1.0)  # Full weight when both attributes match
            else:
                weights.append(factor)  # Lower weight when either attribute doesn't match

    weights = torch.tensor(weights, dtype=probs_gender_ori.dtype, device=probs_gender_ori.device)
    return weights

def plot_in_grid(images, save_to, attribute_indicators=None, gender_preds=None, gender_probs=None, 
                 age_preds=None, age_probs=None):
    """Plot images in a grid for visualization
    
    Args:
        images: torch.Tensor [N,3,H,W]
        save_to: Path to save the visualization
        attribute_indicators: Boolean tensor indicating if attributes were detected
        gender_preds: Predicted gender (0=female, 1=male)
        gender_probs: Gender prediction probabilities
        age_preds: Predicted age (0=young, 1=old)
        age_probs: Age prediction probabilities
    """
    ...
    if attribute_indicators is not None and gender_preds is not None and gender_probs is not None and age_preds is not None and age_probs is not None:
        for i in range(batch_size):
            ...
            if attribute_indicators[i]:
                gender_text = f"G: {'M' if gender_preds[i]==1 else 'F'} ({gender_probs[i]:.2f})"
                age_text = f"A: {'Y' if age_preds[i]==0 else 'O'} ({age_probs[i]:.2f})"
                
                plt.text(x, y - 10, gender_text, color='white', 
                         horizontalalignment='center', verticalalignment='center')
                plt.text(x, y + 10, age_text, color='white',
                         horizontalalignment='center', verticalalignment='center')
...
@torch.no_grad()
def evaluate_process(args, tokenizer, text_encoder, unet, vae, classifier, 
    ...
    for prompt_idx, (prompt, noise) in enumerate(zip(validation_prompts, val_noises)):
        ...
        gender_logits, age_logits = classifier.forward_gender(normalized_images), classifier.forward_age(normalized_images)
        gender_probs = F.softmax(gender_logits, dim=-1)
        age_probs = F.softmax(age_logits, dim=-1)
        
        # Calculate bias metrics - difference between probabilities
        gender_preds = gender_logits.argmax(dim=-1)
        gender_prob_diffs = gender_probs[:, 1] - gender_probs[:, 0]  # male - female
        ...
        gender_preds_all = customized_all_gather(gender_preds, accelerator)
        gender_prob_diffs_all = customized_all_gather(gender_prob_diffs, accelerator)
        gender_probs_all = customized_all_gather(gender_probs, accelerator)
        ...
        if accelerator.is_main_process:
            ...
            plot_in_grid(
                images_all, save_to, 
                torch.ones_like(gender_preds_all, dtype=torch.bool),
                gender_preds_all, 
                gender_probs_all[:, 1],  # Male probability
                age_preds_all,
            ...
            gender_bias = gender_prob_diffs_all.abs().mean().item()
            
            # Calculate gender balance metrics (closer to 0.5 is better)
            gender_ratio = gender_preds_all.float().mean().item()
            gender_balance = abs(gender_ratio - args.target_gender_ratio)
            
            # Calculate age bias metrics
            age_bias = age_prob_diffs_all.abs().mean().item()
            
            # Calculate age balance metrics (closer to target ratio is better)
            age_ratio = age_preds_all.float().mean().item()
            age_balance = abs(1 - age_ratio - args.target_age_ratio)
            
            # Calculate combined bias metric
            combined_bias = gender_bias + age_bias
            
            logs_i["gender_bias"].append(gender_bias)
            logs_i["gender_balanced"].append(1 - gender_balance)  # Higher is better
            logs_i["age_bias"].append(age_bias)
            logs_i["age_balanced"].append(1 - age_balance)  # Higher is better
            logs_i["combined_bias"].append(combined_bias)
...
def apply_grad_hook_attributes(images, targets_gender, preds_gender_ori, probs_gender_ori, 
                               targets_age, preds_age_ori, probs_age_ori, 
                               factor_gender=0.5, factor_age=0.5):
    """Apply gradient hooks to images based on gender and age targets"""
    images_new = []
    for image, target_gender, pred_gender_ori, prob_gender_ori, target_age, pred_age_ori, prob_age_ori in zip(
            images, targets_gender, preds_gender_ori, probs_gender_ori, 
            targets_age, preds_age_ori, probs_age_ori):
        
        # Determine the appropriate factor based on gender and age match
        gender_match = (target_gender == -1) or (target_gender == pred_gender_ori)
        age_match = (target_age == -1) or (target_age == pred_age_ori)
        
        if gender_match and age_match:
            # Both attributes match, keep gradients as is
            grad_hook = make_grad_hook(1.0)
        elif gender_match and not age_match:
            # Only gender matches, reduce gradients for age mismatch
            grad_hook = make_grad_hook(factor_age)
        elif not gender_match and age_match:
            # Only age matches, reduce gradients for gender mismatch
            grad_hook = make_grad_hook(factor_gender)
        else:
            # Neither attribute matches, reduce gradients more
            grad_hook = make_grad_hook(min(factor_gender, factor_age))
        
        # Apply the hook
        hooked_image = image.detach().clone()
        hooked_image.requires_grad_(True)
        hooked_image.register_hook(grad_hook)
        images_new.append(hooked_image.unsqueeze(0))
...
def main():
    ...
    folder_name = (f"CXR-AnoFAIR-GenderAge_BS-{args.train_images_per_prompt_GPU*accelerator.num_processes}_"
    ...
    # 1. Combined Gender-Age classifier
    classifier = GenderAgeClassifier(args.gender_classifier_path, args.age_classifier_path)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, data_idx in enumerate(train_dataloader_idxs[epoch]):
            ...
            if global_step == 0:
                evaluate_process(
                    args, tokenizer, text_encoder, unet, vae, classifier,
                    accelerator, weight_dtype, epoch, global_step
                )
            ...
            if accelerator.is_main_process:
                logs_i = {
                    "loss_da_gender": [],  # Gender distributional alignment loss
                    "loss_da_age": [],     # Age distributional alignment loss
            ...
            with torch.no_grad():
                ...
                # Process generated images for gender and age classification
                resized_images = F.interpolate(images, size=(224, 224), mode='bilinear')
                normalized_images = transforms.Normalize(
                    mean=[0.485, 0.456, 0.406], 
                    std=[0.229, 0.224, 0.225]
                ...
                if accelerator.is_main_process:
                    # Calculate gender bias metrics
                    gender_prob_diffs = gender_probs_all[:, 1] - gender_probs_all[:, 0]  # male - female
                    gender_bias = gender_prob_diffs.abs().mean().item()
                    
                    # Calculate gender balance metrics (closer to target_gender_ratio is better)
                    gender_ratio = gender_preds_all.float().mean().item()
                    gender_balance = abs(gender_ratio - args.target_gender_ratio)
                    ...
                    # Calculate age balance metrics (closer to target_age_ratio is better)
                    age_ratio = (1 - age_preds_all.float().mean().item())  # Percentage of young
                    age_balance = abs(age_ratio - args.target_age_ratio)
                ...
                ################################################
                # Step 2: generate dynamic targets for gender and age alignment
                targets_gender_all, uncertainty_gender_all, targets_age_all, uncertainty_age_all = generate_dynamic_targets_gender_age(
                    gender_probs_all, age_probs_all, 
                    target_gender_ratio=args.target_gender_ratio,
                    target_age_ratio=args.target_age_ratio,
                    w_uncertainty=True
                )
                
                # Apply uncertainty threshold
                targets_gender_all[uncertainty_gender_all > args.uncertainty_threshold] = -1
                targets_age_all[uncertainty_age_all > args.uncertainty_threshold] = -1
                
                # Get local targets
                targets_gender = targets_gender_all[gender_probs.shape[0] * accelerator.local_process_index:
                                                   gender_probs.shape[0] * (accelerator.local_process_index + 1)]
                targets_age = targets_age_all[age_probs.shape[0] * accelerator.local_process_index:
                                             age_probs.shape[0] * (accelerator.local_process_index + 1)]
                
                accelerator.print(f"\tGender targets: {(targets_gender != -1).sum().item()}/{targets_gender.shape[0]}, "
                                 f"Age targets: {(targets_age != -1).sum().item()}/{targets_age.shape[0]}")
                ...
                # Get original gender and age predictions
                gender_logits_ori, age_logits_ori = classifier.forward_gender(normalized_images_ori), classifier.forward_age(normalized_images_ori)
                gender_probs_ori = F.softmax(gender_logits_ori, dim=-1)
                age_probs_ori = F.softmax(age_logits_ori, dim=-1)
                gender_preds_ori = gender_logits_ori.argmax(dim=-1)
                ...
                gender_preds_ori_all = customized_all_gather(gender_preds_ori, accelerator)
                gender_probs_ori_all = customized_all_gather(gender_probs_ori, accelerator)
                ...
                if accelerator.is_main_process:
                    if step % args.train_plot_every_n_iter == 0:
                        ...
                        plot_in_grid(
                            ...
                            torch.ones_like(gender_preds_ori_all, dtype=torch.bool),
                            gender_preds_ori_all, 
                            gender_probs_ori_all[:, 1],  # Male probability
                            age_preds_ori_all,
            ...
            loss_da_gender_i = torch.ones(targets_gender.shape, dtype=weight_dtype, device=accelerator.device) * (-1)  # Gender Distributional Alignment
            loss_da_age_i = torch.ones(targets_age.shape, dtype=weight_dtype, device=accelerator.device) * (-1)        # Age Distributional Alignment
            loss_sp_i = torch.ones(targets_gender.shape, dtype=weight_dtype, device=accelerator.device) * (-1)         # Semantic Preservation
            loss_dp_i = torch.ones(targets_gender.shape, dtype=weight_dtype, device=accelerator.device) * (-1)         # Diagnostic Preservation
            loss_i = torch.ones(targets_gender.shape, dtype=weight_dtype, device=accelerator.device) * (-1)            # Total loss
            
            idxs_i = list(range(targets_gender.shape[0]))
            N_backward = math.ceil(targets_gender.shape[0] / args.train_batch_size)
            for j in range(N_backward):
                ...
                targets_age_ij = targets_age[idxs_ij]
                
                # Get original features for this batch
                clip_feats_ori_ij = clip_feats_ori[idxs_ij]
                dino_feats_ori_ij = dino_feats_ori[idxs_ij]
                gender_preds_ori_ij = gender_preds_ori[idxs_ij]
                gender_probs_ori_ij = gender_probs_ori[idxs_ij]
                ...
                gender_logits_ij, age_logits_ij = classifier.forward_gender(normalized_images_ij), classifier.forward_age(normalized_images_ij)
                gender_probs_ij = F.softmax(gender_logits_ij, dim=-1)
                ...
                # 1. Gender Distributional Alignment Loss
                loss_da_gender_ij = torch.ones(len(idxs_ij), dtype=weight_dtype, device=accelerator.device) * (-1)
                idxs_w_gender_targets = (targets_gender_ij != -1).nonzero().view([-1])
                if len(idxs_w_gender_targets) > 0:
                    loss_da_gender_ij_w_targets = CE_loss(gender_logits_ij[idxs_w_gender_targets], 
                                                         targets_gender_ij[idxs_w_gender_targets])
                    loss_da_gender_ij[idxs_w_gender_targets] = loss_da_gender_ij_w_targets
                
                # 2. Age Distributional Alignment Loss
                loss_da_age_ij = torch.ones(len(idxs_ij), dtype=weight_dtype, device=accelerator.device) * (-1)
                idxs_w_age_targets = (targets_age_ij != -1).nonzero().view([-1])
                if len(idxs_w_age_targets) > 0:
                    loss_da_age_ij_w_targets = CE_loss(age_logits_ij[idxs_w_age_targets], 
                                                     targets_age_ij[idxs_w_age_targets])
                    loss_da_age_ij[idxs_w_age_targets] = loss_da_age_ij_w_targets
                
                # 3. Semantic Preservation Loss (ℒSP)
                ...
                # Calculate dynamic weights based on gender and age alignment
                attribute_indicators = torch.ones(len(idxs_ij), dtype=torch.bool, device=accelerator.device)
                dynamic_weights = gen_dynamic_weights(
                    attribute_indicators, targets_gender_ij, 
                    gender_preds_ori_ij, gender_probs_ori_ij,
                    targets_age_ij, age_preds_ori_ij, age_probs_ori_ij,
                    factor=args.dynamic_weight_factor
                )
                
                # Final loss calculation - combine all components with weights
                loss_ij = (
                    args.distributional_alignment_weight * args.gender_weight * loss_da_gender_ij + 
                    args.distributional_alignment_weight * args.age_weight * loss_da_age_ij + 
                ...
                with torch.no_grad():
                    loss_da_gender_i[idxs_ij] = loss_da_gender_ij.to(loss_da_gender_i.dtype)
                    loss_da_age_i[idxs_ij] = loss_da_age_ij.to(loss_da_age_i.dtype)
            ...
            accelerator.wait_for_everyone()
            loss_da_gender_all = customized_all_gather(loss_da_gender_i, accelerator)
            loss_da_age_all = customized_all_gather(loss_da_age_i, accelerator)
            ...
            # Filter valid losses
            loss_da_gender_all = loss_da_gender_all[loss_da_gender_all != -1]
            loss_da_age_all = loss_da_age_all[loss_da_age_all != -1]
            loss_sp_all = loss_sp_all[loss_sp_all != -1]
            loss_dp_all = loss_dp_all[loss_dp_all != -1]
            loss_all = loss_all[loss_all != -1]
            
            if accelerator.is_main_process:
                logs_i["loss_da_gender"].append(loss_da_gender_all)
                logs_i["loss_da_age"].append(loss_da_age_all)
            ...
            # Process logs
            if accelerator.is_main_process:
                for key in ["loss_da_gender", "loss_da_age", "loss_sp", "loss_dp", "loss"]:
                    if logs_i[key] == []:
                        logs_i.pop(key)
                    else:
                        logs_i[key] = torch.cat(logs_i[key])
                
                for key in ["gender_bias", "gender_balanced", "age_bias", "age_balanced", "combined_bias"]:
                    if logs_i[key] == []:
                        logs_i.pop(key)
